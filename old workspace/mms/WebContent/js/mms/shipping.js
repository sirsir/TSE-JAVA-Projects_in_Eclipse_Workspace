...
$(document).ready(function() {
	...
	$("#find").click(function() {
		window.location = "shipping.htm?start=" + $("#start").val() + "&end=" + $("#end").val();
	});
	...
	$("#add-btn").click(function() {
		...
		$.get("rest/dateCodeWs?type=shipping&dcode=" + dcode, function(xml) {
...
function selectRow(id) {
	if(shpId)
		$("#" + shpId).removeClass("selected-row");
	shpId = id;
	$("#" + shpId).addClass("selected-row");
...
function openDlg(type) {
	if(type == "add") {
		...
		addDlg.dialog("option", "title", "Add shipping plan");
		addDlg.dialog("open");
	}
	else if(type == "update") {
		$(".ui-widget-header").css("background", "#DDFA9D");
		addDlg.dialog("option", "title", "Update shipping plan");
		addDlg.dialog("open");
	}
}
		...
		success:function(result) {
			if(numberRegex.test(result)) {
				$("#row").append("<tr id='" + result + "'><td>" + date + "</td><td>" + no + "</td><td id='p" + prdId + "'>" + 
						prdName + "</td><td class='td-number'>" + planQty + "</td><td class='td-number'>" + (shipQty>0?shipQty:"") + "</td></tr>");
...
function perpareUpdate() {
	...
	$("#shipQty").val($(e).children().eq(4).html());
	openDlg("update");
}
	...
	$.ajax({
		...
		success:function(result) {
			if(result == "true") {
				...
				$(e).children().eq(4).html(shipQty);
				addDlg.dialog("close");
			}
			else alert("Cannot update Shipping Plan No. " + $("#no").val() + "!");
		}
	});
}

function del() {
	var no = $("#" + shpId).children().eq(1).html();
	if(confirm("Do you want to delete Shipping Plan No. " + no + "?")) {
		$.ajax({
			type:"DELETE",
			url:"rest/shippingWs",
			data: {
			...
			success:function(result) {
				...
				else alert("Cannot remove Shipping Plan No. " + no + "!");
			}
		});
	}
}